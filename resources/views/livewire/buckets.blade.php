                    ...
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table class="w-full divide-y divide-gray-200 text-center">
                            <thead class="bg-green-600 text-white font-bold capitalize">
                                ...
                                <th class="px-4 py-2">Community</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                @forelse($buckets as $bucket)
                                    <tr>
                                        <td class=" px-6 py-4 whitespace-nowrap">{{ $bucket->id }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap ">
                                            {{ $bucket->city }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap ">
                                            {{ Str::words($bucket->address, 4, '...') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap ">
                                            {{ $bucket->created_at }}

                                        </td>
                                        <td>
                                            <x-button wire:click="showEditDataModal({{ $bucket->id }})">
                                            ...
                                            <x-button-delete wire:click="$emit('deleteData',{{ $bucket->id }})">
                        ...
                        <div class="m-2 p-2">{{ $buckets->links() }}</div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-8 divide-y divide-gray-200 w-full mt-10">
                        <form enctype="multipart/form-data" autocomplete="off">
                            <div class="sm:col-span-6">
                                ...
                                <div class="mt-1">
                                    <input type="text" id="description" wire:model.lazy="description"
                                        name="description"
                                        class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 mb-2" />
                                </div>
                                @error('description')
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <div class="mt-1">
                                    <input type="text" id="community" wire:model.lazy="community" name="community"
                                        class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 mb-2" />
                                </div>
                                @error('community')
                                    <span class="text-red-400">{{ $message }}</span>
            ...
            Livewire.on('deleteData', catId => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.emitTo('bucket', 'delete', catId)
                        Swal.fire(
                            'Deleted!',
